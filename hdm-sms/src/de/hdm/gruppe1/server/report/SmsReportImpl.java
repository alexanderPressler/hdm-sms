...
public class SmsReportImpl extends RemoteServiceServlet implements SmsReport {
	...
	protected Sms getSmsVerwaltung() {
		return this.stuecklistenVerwaltung;
	}

	/**
	 * Setzen des zugehörigen Baugrppen-Objekte.
	 */
	public void setBaugruppe(Baugruppe b) {

		this.stuecklistenVerwaltung.setBaugruppe(b);

	}

	protected void addImprint(Report r) {
		/*
		 * Das Impressum soll wesentliche Informationen über die Baugruppen enthalten.
		 */
		Baugruppe baugruppe = this.stuecklistenVerwaltung.getBaugruppe();
		/*
...
//		imprint.addSubParagraph(new SimpleParagraph(baugruppe.getId()));
//		imprint.addSubParagraph(new SimpleParagraph(baugruppe.getUser()));

		// Das eigentliche Hinzufügen des Impressums zum Report.
		r.setImprint(imprint);

	/*
	 * Erstellen der Struckturstücklisten 
	 */
	}

	public StrukturStuecklisteReport createStrukturStuecklisteReport(Baugruppe b)
			throws IllegalArgumentException {

		if (this.getSmsVerwaltung() == null)
			return null;
		...
		result.setTitle("Strukturstueckliste der Elementen");
		...
		/****************************************************************
		 * Ab hier erfolgt ein zeilenweises Hinzufügen von der Struckturstueckliste
		 * 
		 *********************************************************************/

		/*
		 * Zunächst legen wir eine Kopfzeile für die
		 * Strukturstuecklisten-Tabelle an.
		 */
		...
		Vector<Baugruppe> baugruppen = this.stuecklistenVerwaltung.getAllBaugruppen();
		...
		for (Baugruppe bg : baugruppen) {
			...
			// baugruppeRow.addColumn(new
			// Column(String.valueOf(this.verwaltung.setBaugruppe())));
	...
	public MaterialBedarfReport createMaterialBedarfReport()
		...
		for (Baugruppe bg : baugruppen) {
			...
			baugruppeRow.addColumn(new Column(String.valueOf(bg.getId())));
			baugruppeRow.addColumn(new Column(String.valueOf(bg.getName())));
			baugruppeRow.addColumn(new Column(String.valueOf(this.stuecklistenVerwaltung
					.getAllBaugruppen())));
			result.addRow(baugruppeRow);
		}
		return result;
